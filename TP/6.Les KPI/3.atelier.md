# 📊 GUIDE SSAS - VERSION SIMPLIFIÉE
## Création du Cube SSAS pour Démo KPI

---

## 🎯 ARCHITECTURE SIMPLIFIÉE

```
DW_KPI_Simple
├─ DimTime (1,096 jours)
├─ DimProduct (20 produits, 3 catégories)
├─ DimCustomer (50 clients)
└─ FactSales (~50,000 transactions)
```

**Parfait pour démontrer TOUS les KPI !**

---

## ÉTAPE 1 : CRÉER LE PROJET SSAS

### 1.1 Nouveau Projet

```
Visual Studio / SSDT
└─ File > New > Project
   └─ Analysis Services Multidimensional Project
      └─ Name: KPI_Demo_Simple
      └─ Location: C:\SSAS\Projects
      └─ OK
```

---

## ÉTAPE 2 : DATA SOURCE

### 2.1 Ajouter la Source

```
Solution Explorer
└─ Data Sources (clic droit)
   └─ New Data Source
      └─ Next
      └─ New... (Connection Manager)
         ├─ Server name: localhost (ou votre serveur)
         ├─ Database: DW_KPI_Simple
         ├─ Test Connection → Success!
         └─ OK
      └─ Next
      └─ Impersonation: Use the service account
      └─ Next
      └─ Data source name: DW_KPI_Simple
      └─ Finish
```

---

## ÉTAPE 3 : DATA SOURCE VIEW

### 3.1 Créer la DSV

```
Data Source Views (clic droit)
└─ New Data Source View
   └─ Next
   └─ Select: DW_KPI_Simple
   └─ Next
   └─ Available objects:
      ☑ DimTime
      ☑ DimProduct
      ☑ DimCustomer
      ☑ FactSales
   └─ Add > (4 tables dans included objects)
   └─ Next
   └─ Name: DSV_KPI_Simple
   └─ Finish
```

### 3.2 Vérifier les Relations

Le diagramme doit montrer :
```
DimTime ──────┐
              │
DimProduct ───┼──> FactSales
              │
DimCustomer ──┘
```

Si relations manquantes, les ajouter manuellement :
- Glisser DimTime.TimeKey → FactSales.TimeKey
- Glisser DimProduct.ProductKey → FactSales.ProductKey
- Glisser DimCustomer.CustomerKey → FactSales.CustomerKey

---

## ÉTAPE 4 : CRÉER LES DIMENSIONS

### 4.1 Dimension Temps (DimTime)

```
Dimensions (clic droit) > New Dimension
└─ Next
└─ Use an existing table
└─ Next
└─ Main table: DimTime
└─ Key columns: TimeKey
└─ Name column: FullDate
└─ Next
└─ Dimension type: Time
└─ Calendar time period: Date
└─ Next
└─ Attributes à sélectionner:
   ☑ Full Date
   ☑ Year
   ☑ Year Month
   ☑ Quarter Name
   ☑ Month Name
   ☑ Day Of Month
└─ Next
└─ Dimension name: Dim Time
└─ Finish
```

**Configuration des Hiérarchies :**

Dans Dimension Designer :
```
Hierarchies (glisser-déposer) :
└─ Calendar (nouvelle hiérarchie)
   ├─ Year
   ├─ Quarter Name
   ├─ Month Name
   └─ Full Date
```

**Propriétés importantes :**
```
DimTime (dimension)
└─ Type: Time
└─ Calendar: Calendar hierarchy

Year (attribut)
└─ AttributeHierarchyVisible: True
└─ Type: Years

Full Date (attribut)
└─ Type: Date
└─ KeyColumns: TimeKey
└─ NameColumn: FullDate
```

### 4.2 Dimension Produit (DimProduct)

```
New Dimension
└─ Next
└─ Use an existing table
└─ Next
└─ Main table: DimProduct
└─ Key: ProductKey
└─ Name: ProductName
└─ Next
└─ Attributes:
   ☑ Product Name
   ☑ Product Category
   ☑ Product Code
└─ Next
└─ Dimension name: Dim Product
└─ Finish
```

**Créer la hiérarchie produit :**
```
Product Hierarchy
├─ Product Category
└─ Product Name
```

### 4.3 Dimension Client (DimCustomer)

```
New Dimension
└─ Main table: DimCustomer
└─ Key: CustomerKey
└─ Name: CustomerName
└─ Attributes:
   ☑ Customer Name
   ☑ Customer Segment
   ☑ Region
   ☑ Country
└─ Dimension name: Dim Customer
└─ Finish
```

**Créer la hiérarchie géographique :**
```
Geography
├─ Country
├─ Region
└─ Customer Name
```

---

## ÉTAPE 5 : CRÉER LE CUBE

### 5.1 Nouveau Cube

```
Cubes (clic droit) > New Cube
└─ Next
└─ Use existing tables
└─ Next
└─ Measure group tables:
   ☑ FactSales
└─ Next
└─ Mesures à sélectionner:
   ☑ Order Quantity (Sum)
   ☑ Returned Quantity (Sum)
   ☑ Customer Satisfaction Score (Average)
   ☑ Sales Target (Sum)
   ☑ Margin Target (Sum)
   ☑ Gross Sales Amount (Sum)
   ☑ Net Sales Amount (Sum)
   ☑ Total Cost (Sum)
   ☑ Gross Profit (Sum)
   ☑ Fact Sales Count (Count)
└─ Next
└─ Dimensions existantes:
   ☑ Dim Time
   ☑ Dim Product
   ☑ Dim Customer
└─ Next
└─ Cube name: KPI Simple Cube
└─ Finish
```

### 5.2 Configurer les Mesures

Dans Cube Designer, onglet **Cube Structure** :

**Renommer les mesures :**
```
Order Quantity → OrderQuantity
Returned Quantity → ReturnedQuantity
Customer Satisfaction Score → OverallSatisfactionScore
Gross Sales Amount → GrossSalesAmount
Net Sales Amount → NetSalesAmount
Total Cost → TotalCost
Gross Profit → GrossProfit
Fact Sales Count → TransactionCount
```

**Format des mesures :**
```
NetSalesAmount
└─ FormatString: #,##0

GrossProfit
└─ FormatString: #,##0

OverallSatisfactionScore
└─ FormatString: #.0
└─ AggregateFunction: Average
```

---

## ÉTAPE 6 : MESURES CALCULÉES

Onglet **Calculations**, cliquer **New Calculated Member**.

### 6.1 Marge Pourcentage

```mdx
CREATE MEMBER CURRENTCUBE.[Measures].[Margin Percentage]
AS 
  -- ATTENTION: Pas de NULLIF en MDX!
  -- Utiliser IIF pour éviter division par zéro
  IIF(
    [Measures].[NetSalesAmount] = 0,
    NULL,
    [Measures].[GrossProfit] / [Measures].[NetSalesAmount] * 100
  ),
FORMAT_STRING = "#,##0.00",
VISIBLE = 1;
```

### 6.2 Taux d'Atteinte des Ventes

```mdx
CREATE MEMBER CURRENTCUBE.[Measures].[Sales Achievement Rate]
AS 
  IIF(
    [Measures].[SalesTarget] = 0,
    NULL,
    [Measures].[NetSalesAmount] / [Measures].[SalesTarget]
  ),
FORMAT_STRING = "Percent",
VISIBLE = 1;
```

### 6.3 Ventes Période Précédente (PY)

```mdx
CREATE MEMBER CURRENTCUBE.[Measures].[Sales PY]
AS 
  (
    ParallelPeriod(
      [Dim Time].[Calendar].[Year],
      1,
      [Dim Time].[Calendar].CurrentMember
    ),
    [Measures].[NetSalesAmount]
  ),
FORMAT_STRING = "#,##0",
VISIBLE = 1;
```

### 6.4 Croissance YoY

```mdx
CREATE MEMBER CURRENTCUBE.[Measures].[Sales Growth YoY]
AS 
  IIF(
    [Measures].[Sales PY] = 0,
    NULL,
    ([Measures].[NetSalesAmount] - [Measures].[Sales PY]) / [Measures].[Sales PY]
  ),
FORMAT_STRING = "Percent",
VISIBLE = 1;
```

### 6.5 Panier Moyen

```mdx
CREATE MEMBER CURRENTCUBE.[Measures].[Average Basket]
AS 
  IIF(
    [Measures].[TransactionCount] = 0,
    NULL,
    [Measures].[NetSalesAmount] / [Measures].[TransactionCount]
  ),
FORMAT_STRING = "#,##0.00",
VISIBLE = 1;
```

### 6.6 Taux de Retour

```mdx
CREATE MEMBER CURRENTCUBE.[Measures].[Return Rate]
AS 
  IIF(
    [Measures].[OrderQuantity] = 0,
    0,
    [Measures].[ReturnedQuantity] / [Measures].[OrderQuantity]
  ),
FORMAT_STRING = "Percent",
VISIBLE = 1;
```

### 6.7 Satisfaction Achievement

```mdx
CREATE MEMBER CURRENTCUBE.[Measures].[Satisfaction Achievement]
AS 
  -- Objectif: 4.0/5
  IIF(
    [Measures].[OverallSatisfactionScore] = NULL,
    NULL,
    [Measures].[OverallSatisfactionScore] / 4.0
  ),
FORMAT_STRING = "Percent",
VISIBLE = 1;
```

---

## ÉTAPE 7 : CRÉER LES KPI

Onglet **KPIs**, cliquer **New KPI**.

### 7.1 KPI Global Performance (Niveau Stratégique)

```
Name: Global_Performance
Associated measure group: Fact Sales

VALUE Expression:
-- Composite pondéré (sans NULLIF!)
IIF(
  [Measures].[SalesTarget] = 0 OR [Measures].[Sales PY] = 0,
  0,
  (
    [Measures].[Sales Achievement Rate] * 0.4 +
    ([Measures].[Margin Percentage] / 45.0) * 0.3 +
    ([Measures].[OverallSatisfactionScore] / 5.0) * 0.2 +
    IIF([Measures].[Sales Growth YoY] < 0, 0, [Measures].[Sales Growth YoY]) * 0.1
  )
)

GOAL Expression:
1.0

STATUS Expression:
CASE
  WHEN KpiValue("Global_Performance") >= 0.95 THEN 1
  WHEN KpiValue("Global_Performance") >= 0.85 THEN 0
  ELSE -1
END

TREND Expression:
-- Comparer au trimestre précédent
IIF(
  (
    ParallelPeriod(
      [Dim Time].[Calendar].[Quarter],
      1,
      [Dim Time].[Calendar].CurrentMember
    ),
    KpiValue("Global_Performance")
  ) = 0,
  0,
  CASE
    WHEN KpiValue("Global_Performance") > 
         (
           ParallelPeriod(
             [Dim Time].[Calendar].[Quarter],
             1,
             [Dim Time].[Calendar].CurrentMember
           ),
           KpiValue("Global_Performance")
         ) * 1.03 THEN 1
    WHEN KpiValue("Global_Performance") < 
         (
           ParallelPeriod(
             [Dim Time].[Calendar].[Quarter],
             1,
             [Dim Time].[Calendar].CurrentMember
           ),
           KpiValue("Global_Performance")
         ) * 0.97 THEN -1
    ELSE 0
  END
)

STATUS Graphic: Gauge
TREND Graphic: Standard Arrow

Display Folder: 1 - Strategic KPIs
```

### 7.2 KPI Commercial Performance (Niveau Tactique)

```
Name: Commercial_Performance
Associated measure group: Fact Sales

VALUE Expression:
IIF(
  [Measures].[SalesTarget] = 0,
  0,
  (
    [Measures].[Sales Achievement Rate] * 0.5 +
    ([Measures].[Margin Percentage] / 45.0) * 0.3 +
    ([Measures].[Average Basket] / 200.0) * 0.2
  )
)

GOAL Expression:
1.0

STATUS Expression:
CASE
  WHEN KpiValue("Commercial_Performance") >= 0.95 THEN 1
  WHEN KpiValue("Commercial_Performance") >= 0.85 THEN 0
  ELSE -1
END

TREND Expression:
IIF(
  (
    ParallelPeriod(
      [Dim Time].[Calendar].[Month],
      1,
      [Dim Time].[Calendar].CurrentMember
    ),
    KpiValue("Commercial_Performance")
  ) = 0,
  0,
  CASE
    WHEN KpiValue("Commercial_Performance") > 
         (ParallelPeriod([Dim Time].[Calendar].[Month], 1), KpiValue("Commercial_Performance")) * 1.02 THEN 1
    WHEN KpiValue("Commercial_Performance") < 
         (ParallelPeriod([Dim Time].[Calendar].[Month], 1), KpiValue("Commercial_Performance")) * 0.98 THEN -1
    ELSE 0
  END
)

STATUS Graphic: Traffic Light
TREND Graphic: Standard Arrow

Display Folder: 2 - Tactical KPIs
Parent KPI: Global_Performance
Weight: 0.5
```

### 7.3 KPI Customer Quality (Niveau Tactique)

```
Name: Customer_Quality
Associated measure group: Fact Sales

VALUE Expression:
-- Qualité client composite (Satisfaction + Retours)
(
  ([Measures].[OverallSatisfactionScore] / 5.0) * 0.6
  +
  (1.0 - [Measures].[Return Rate]) * 0.4
)

GOAL Expression:
0.90

STATUS Expression:
CASE
  WHEN KpiValue("Customer_Quality") >= 0.90 THEN 1
  WHEN KpiValue("Customer_Quality") >= 0.80 THEN 0
  ELSE -1
END

TREND Expression:
IIF(
  (ParallelPeriod([Dim Time].[Calendar].[Month], 1), KpiValue("Customer_Quality")) = 0,
  0,
  CASE
    WHEN KpiValue("Customer_Quality") > 
         (ParallelPeriod([Dim Time].[Calendar].[Month], 1), KpiValue("Customer_Quality")) * 1.02 THEN 1
    WHEN KpiValue("Customer_Quality") < 
         (ParallelPeriod([Dim Time].[Calendar].[Month], 1), KpiValue("Customer_Quality")) * 0.98 THEN -1
    ELSE 0
  END
)

STATUS Graphic: Faces
TREND Graphic: Standard Arrow

Display Folder: 2 - Tactical KPIs
Parent KPI: Global_Performance
Weight: 0.3
```

### 7.4 KPI Sales by Category (Niveau Opérationnel)

```
Name: Sales_By_Category
Associated measure group: Fact Sales

VALUE Expression:
[Measures].[Sales Achievement Rate]

GOAL Expression:
1.0

STATUS Expression:
CASE
  WHEN KpiValue("Sales_By_Category") >= 1.05 THEN 1
  WHEN KpiValue("Sales_By_Category") >= 0.95 THEN 0
  ELSE -1
END

TREND Expression:
IIF(
  (ParallelPeriod([Dim Time].[Calendar].[Month], 1), [Measures].[NetSalesAmount]) = 0,
  0,
  CASE
    WHEN [Measures].[Sales Growth YoY] > 0.15 THEN 1
    WHEN [Measures].[Sales Growth YoY] < 0.10 THEN -1
    ELSE 0
  END
)

STATUS Graphic: Road Signs
TREND Graphic: Standard Arrow

Display Folder: 3 - Operational KPIs
Parent KPI: Commercial_Performance
```

### 7.5 KPI Margin by Category (Niveau Opérationnel)

```
Name: Margin_By_Category
Associated measure group: Fact Sales

VALUE Expression:
-- Marge réelle vs marge cible
IIF(
  [Dim Product].[Product Category].CurrentMember.Level.Ordinal = 0,
  -- All Products: objectif global 45%
  [Measures].[Margin Percentage] / 45.0,
  -- Par catégorie avec objectifs différents
  IIF(
    [Dim Product].[Product Category].CurrentMember.Name = "Électronique",
    [Measures].[Margin Percentage] / 42.0,
    IIF(
      [Dim Product].[Product Category].CurrentMember.Name = "Électroménager",
      [Measures].[Margin Percentage] / 38.0,
      [Measures].[Margin Percentage] / 48.0  -- Meubles
    )
  )
)

GOAL Expression:
1.0

STATUS Expression:
CASE
  WHEN KpiValue("Margin_By_Category") >= 1.0 THEN 1
  WHEN KpiValue("Margin_By_Category") >= 0.9 THEN 0
  ELSE -1
END

TREND Expression:
IIF(
  (ParallelPeriod([Dim Time].[Calendar].[Month], 1), KpiValue("Margin_By_Category")) = 0,
  0,
  CASE
    WHEN KpiValue("Margin_By_Category") > 
         (ParallelPeriod([Dim Time].[Calendar].[Month], 1), KpiValue("Margin_By_Category")) * 1.02 THEN 1
    WHEN KpiValue("Margin_By_Category") < 
         (ParallelPeriod([Dim Time].[Calendar].[Month], 1), KpiValue("Margin_By_Category")) * 0.98 THEN -1
    ELSE 0
  END
)

STATUS Graphic: Gauge
TREND Graphic: Standard Arrow

Display Folder: 3 - Operational KPIs
Parent KPI: Commercial_Performance
```

### 7.6 KPI Customer Satisfaction (Niveau Opérationnel)

```
Name: Customer_Satisfaction
Associated measure group: Fact Sales

VALUE Expression:
[Measures].[OverallSatisfactionScore]

GOAL Expression:
4.0

STATUS Expression:
CASE
  WHEN KpiValue("Customer_Satisfaction") >= 4.0 THEN 1
  WHEN KpiValue("Customer_Satisfaction") >= 3.5 THEN 0
  ELSE -1
END

TREND Expression:
IIF(
  (ParallelPeriod([Dim Time].[Calendar].[Month], 1), [Measures].[OverallSatisfactionScore]) = 0,
  0,
  CASE
    WHEN [Measures].[OverallSatisfactionScore] > 
         (ParallelPeriod([Dim Time].[Calendar].[Month], 1), [Measures].[OverallSatisfactionScore]) * 1.05 THEN 1
    WHEN [Measures].[OverallSatisfactionScore] < 
         (ParallelPeriod([Dim Time].[Calendar].[Month], 1), [Measures].[OverallSatisfactionScore]) * 0.95 THEN -1
    ELSE 0
  END
)

STATUS Graphic: Faces
TREND Graphic: Standard Arrow

Display Folder: 3 - Operational KPIs
Parent KPI: Customer_Quality
```

### 7.7 KPI Return Rate (Niveau Opérationnel)

```
Name: Return_Rate_KPI
Associated measure group: Fact Sales

VALUE Expression:
-- INVERSÉ: Plus bas est mieux!
-- Objectif: max 3% de retours
IIF(
  [Measures].[OrderQuantity] = 0,
  0,
  1 - ([Measures].[Return Rate] / 0.03)
)

GOAL Expression:
1.0

STATUS Expression:
-- Inversé: taux bas = vert
CASE
  WHEN [Measures].[Return Rate] <= 0.02 THEN 1   -- Moins de 2%: Excellent
  WHEN [Measures].[Return Rate] <= 0.03 THEN 0   -- 2-3%: OK
  ELSE -1                                          -- Plus de 3%: Rouge
END

TREND Expression:
IIF(
  (ParallelPeriod([Dim Time].[Calendar].[Month], 1), [Measures].[Return Rate]) = 0,
  0,
  CASE
    -- Tendance INVERSÉE: baisse = positif!
    WHEN [Measures].[Return Rate] < 
         (ParallelPeriod([Dim Time].[Calendar].[Month], 1), [Measures].[Return Rate]) * 0.9 THEN 1
    WHEN [Measures].[Return Rate] > 
         (ParallelPeriod([Dim Time].[Calendar].[Month], 1), [Measures].[Return Rate]) * 1.1 THEN -1
    ELSE 0
  END
)

STATUS Graphic: Reversed Gauge
TREND Graphic: Standard Arrow

Display Folder: 3 - Operational KPIs
Parent KPI: Customer_Quality
```

---

## ÉTAPE 8 : DEPLOY ET PROCESS

### 8.1 Deploy

```
Solution Explorer
└─ KPI_Demo_Simple (clic droit)
   └─ Properties
      └─ Deployment
         ├─ Server: localhost
         ├─ Database: KPI_Demo_SSAS_Simple
         └─ OK
   └─ Deploy
```

Attendre "Deploy succeeded"

### 8.2 Process

```
Solution Explorer
└─ KPI Simple Cube.cube (clic droit)
   └─ Process
      └─ Process Full
      └─ OK
      └─ Run
```

Attendre "Process succeeded"

---

## ÉTAPE 9 : TESTER DANS LE BROWSER

### 9.1 Browser SSAS

```
Cube Designer
└─ Onglet Browser
   └─ Reconnect (si nécessaire)
```

**Test 1 : Mesures de base**
```
Glisser:
- [Measures].[NetSalesAmount] dans Drop Data Fields Here
- [Dim Time].[Calendar].[Year] dans Drop Row Fields Here

Résultat attendu:
2022: ~8,000,000
2023: ~10,000,000
2024: ~12,000,000
```

**Test 2 : KPI Global**
```
Glisser:
- [KPIs].[Global_Performance].[Value] dans Data
- [KPIs].[Global_Performance].[Status] dans Data
- [KPIs].[Global_Performance].[Trend] dans Data
- [Dim Time].[Calendar].[Quarter Name] dans Rows

Résultat: Voyez les indicateurs visuels!
```

### 9.2 Test MDX

Nouvelle requête MDX :

```mdx
SELECT 
  {
    [Measures].[NetSalesAmount],
    [Measures].[GrossProfit],
    [Measures].[Margin Percentage],
    [Measures].[Sales Achievement Rate]
  } ON COLUMNS,
  
  [Dim Time].[Calendar].[Year].Members ON ROWS

FROM [KPI Simple Cube]
```

---

## ÉTAPE 10 : EXCEL

### 10.1 Connexion

```
Excel
└─ Données > Obtenir des données externes
   └─ From Analysis Services
      └─ Server: localhost
      └─ Database: KPI_Demo_SSAS_Simple
      └─ Cube: KPI Simple Cube
      └─ Créer PivotTable
```

### 10.2 Dashboard KPI

**Créer 3 tableaux :**

**Tableau 1 : KPI Stratégiques**
```
Lignes: [Dim Time].[Calendar].[Quarter Name]
Valeurs: 
  - [KPIs].[Global_Performance].[Value]
  - [KPIs].[Global_Performance].[Status]
```

**Tableau 2 : KPI par Catégorie**
```
Lignes: [Dim Product].[Product Category]
Valeurs:
  - [Measures].[NetSalesAmount]
  - [Measures].[Margin Percentage]
  - [KPIs].[Margin_By_Category].[Status]
```

**Tableau 3 : Évolution Temporelle**
```
Lignes: [Dim Time].[Calendar].[Month Name]
Valeurs:
  - [Measures].[NetSalesAmount]
  - [Measures].[Sales Growth YoY]
  - [KPIs].[Commercial_Performance].[Status]
```

---

## 🎯 POINTS CLÉS MDX

### ⚠️ PAS DE NULLIF EN MDX!

**❌ ERREUR (ne fonctionne pas) :**
```mdx
[Measures].[A] / NULLIF([Measures].[B], 0)
```

**✅ CORRECT (utiliser IIF) :**
```mdx
IIF(
  [Measures].[B] = 0,
  NULL,
  [Measures].[A] / [Measures].[B]
)
```

### Autres pièges à éviter :

1. **Division par zéro :** Toujours tester avec IIF
2. **Comparaison NULL :** Utiliser `IS NULL` ou `= 0`
3. **ParallelPeriod :** Peut retourner NULL, tester avant utilisation
4. **Format_String :** Toujours entre guillemets

---

## ✅ CHECKLIST FINALE

```
☐ Base de données créée et populée
☐ Projet SSAS créé
☐ Data Source configurée
☐ Data Source View avec 4 tables
☐ 3 Dimensions créées avec hiérarchies
☐ Cube créé avec toutes les mesures
☐ 7+ mesures calculées (sans NULLIF!)
☐ 7 KPI hiérarchiques configurés
☐ Deploy réussi
☐ Process Full réussi
☐ Browser affiche des données
☐ Excel se connecte au cube
☐ KPI affichent des indicateurs visuels
```

---

## 🎉 FÉLICITATIONS !

Vous avez créé un **cube SSAS complet** avec :
- ✅ Architecture simplifiée (3 dimensions)
- ✅ Données réalistes (~50K transactions)
- ✅ Mesures calculées MDX (sans NULLIF!)
- ✅ KPI hiérarchiques (3 niveaux)
- ✅ Indicateurs visuels (gauges, feux, faces)
- ✅ Intégration Excel

**Parfait pour démontrer toutes les capacités KPI de SSAS !** 🚀